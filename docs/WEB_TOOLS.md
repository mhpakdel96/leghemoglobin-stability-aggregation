...
Used in place of Protein-Sol after the latter failed twice.

| Item | Value |
|---|---|
| Purpose | Sequence-based solubility prediction, trained on *E. coli* soluble-expression data |
| Input | Eight FASTA sequences generated by `scripts/analysis/solubility/make_fasta.py` |
| Date run | 2026-08-16 |
| Output | `results/soluprot/soluprot_results.csv` |

Higher scores indicate higher predicted solubility; the classification
threshold is 0.5. All eight variants scored between 0.766 and 0.820 and are
therefore all predicted soluble. The tool is a classifier, not a calibrated
quantitative scale, so the sign of each change is reported rather than a
precise ranking. Results and interpretation: `docs/RESULTS.md` section 2c.
