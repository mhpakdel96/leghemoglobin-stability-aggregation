...
## 8b. Sequence-Based Solubility

SoluProt, run on the eight FASTA sequences generated by
`scripts/analysis/solubility/make_fasta.py`. The tool is trained on
soluble-expression data from *E. coli*, which makes it relevant to the host
comparison and independent of every structural method used here.

Higher scores indicate higher predicted solubility; the classification
threshold is 0.5. All eight variants scored 0.766–0.820 and are therefore all
predicted soluble. SoluProt is a classifier rather than a calibrated
quantitative scale, so the sign of each change is reported and the precise
ranking is not.

Protein-Sol was the tool originally intended for this step. It returned a
server error (`No space left on device`) on both attempts.
